...
class SimpleCNN(nn.Module):
    def __init__(self, n_in=[4,84,84], conv_channels=[32, 64, 64],
        ...
        for c, k, s in zip(conv_channels, conv_kernels, conv_strides):
            ...
            self.conv_layers.append( nn.Conv2d(in_channels = c0, out_channels = c, kernel_size = k, stride = s) )
            # append nn.ReLU layer
            self.conv_layers.append( nn.ReLU(self.conv_layers) )
            ### Your Code Ends >>>
    ...
    def forward(self, x, head=None):
        ...
        #h = x.shape[2]
        x = x.view(x.shape[0], -1) #x.view(-1, 32*32)
